...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        # get instance_token to build up 2d-3d bbox correspondency
        # can be simplified, step-by-step operation for reading
        instances = []
        for box in boxes:
            # box.token is anno token pointing to anno itself
            anno_box = nusc.get('sample_annotation', box.token)
            instances.append(anno_box['instance_token'])
        ...
        if not test:
            ...
            valid_flag = np.array(
                [(anno["num_lidar_pts"] + anno["num_radar_pts"]) > 0
                    for anno in annotations], dtype=bool,
                ).reshape(-1)
            ...
            # annotations in same order and length as instances
            assert len(gt_boxes) == len(annotations), \
                f"{len(gt_boxes)}, {len(annotations)}"
            # add instance_token into pkl
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            ...
            coco_infos = get_2d_boxes(
                ...
                # TODO: visibility filtering, no arg setting available
                visibilities=["4"], # default keys include ""
                mono3d=mono3d,
            )
...
def get_2d_boxes(nusc, sample_data_token: str, visibilities: List[str], mono3d=True):
    ...
    # Enable 2d bbox of 'camera' to correspond to 3d bbox
    # who refers to particular 'instance' in 'sample'
    camera = sd_rec['channel']
    ...
    for ann_rec in ann_recs:
        ...
        if mono3d and (repro_rec is not None):
            ...
            # add camera and instance into coco 2d annotation
            repro_rec['camera'] = camera
            repro_rec['instance'] = instance_token
            # use tokens(str, nus) rather than index(int, coco) as id
            repro_rec['id'] = ann_rec["token"]
