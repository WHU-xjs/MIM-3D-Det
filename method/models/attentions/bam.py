...
class BAM(BaseModule):
    def __init__(self, in_channels, depthwise_spatial=False):
        super(BAM, self).__init__()
        self.channel_att = ChannelGate(in_channels)
        if not depthwise_spatial:
            self.spatial_att = SpatialGate(in_channels)
        else:
            self.spatial_att = DepthwiseSeparableSpatialGate(in_channels)
...
@ATTENTIONS.register_module()
class BAM_NO_SPT(BaseModule):
    def __init__(self, in_channels):
        super(BAM_NO_SPT, self).__init__()
        self.channel_att = ChannelGate(in_channels)
    def forward(self,in_tensor):
...
@ATTENTIONS.register_module()
class BAM_NO_CHN(BaseModule):
    def __init__(self, in_channels, depthwise_spatial=False):
        super(BAM_NO_CHN, self).__init__()
        if not depthwise_spatial:
            self.spatial_att = SpatialGate(in_channels)
        else:
            self.spatial_att = DepthwiseSeparableSpatialGate(in_channels)
